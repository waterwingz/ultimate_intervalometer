...
-- configure HDR shooting
    ...
    if ( hdr_mode == 2 ) then tv_offset = hdr_offset else tv_offset = 0 end
    if ( hdr_mode == 3 ) then sv_offset = hdr_offset else sv_offset = 0 end
    if ( hdr_mode == 4 ) then av_offset = hdr_offset else av_offset = 0 end
...
function lprintf(...)
    if ( log_mode > 1 ) then
        ...
        if ( log~=nil) then
                ...
                if ( elapsed_days == 0 ) then ss = "Day -- " end
...
function printf(...)
    if ( log_mode ~= 0) then
        local str=string.format(...)
        if (( log_mode == 1) or (log_mode == 3)) then print(string.sub(str,1,88)) end
        lprintf(...)
    end
end
    ...
    if (display_mode>0) then                                            -- display control enable?
        if ( display_hold_timer>0) then                                 -- do nothing until display hold timer expires
            ...
            if (display_mode==2) then                                   -- backlight on/off  (allow to happen every time called)
                if (display_active ~= newstate) then printf("set backlight %s",st) end
                sleep(1000)
            ...
            elseif (display_active ~= newstate ) then
                ...
                elseif (display_mode==4) then                           -- switch to PLAYBACK and turn off display
                    ...
                    if (newstate==true) then switch_mode(SHOOTING) else switch_mode(PLAYBACK) end
                    set_lcd_display(newstate)
...
function get_start_stop_times()
    ...
    if ((dawn_mode) and ( start_time > dawn)) then day_time_start=dawn-(time_offset*60) else day_time_start=start_time end
    if ((dusk_mode) and ( stop_time < dusk )) then day_time_stop =dusk+(time_offset*60) else day_time_stop =stop_time  end
                    ...
                    if (hdr_mode == 0) then                         -- single shot mode
                        ...
                        if ( hdr_mode == 5 ) then                   -- burst mode ?
                            ...
                            for i = 0-hdr_shots, hdr_shots, 1 do
                                ...
                                set_prop(pTV,tv)
                                if( propset > 3) then set_prop(pTV2,tv) end
                            ...
                            if (hdr_mode == 1) then set_ev(ev96current) end
                        end
                    end
